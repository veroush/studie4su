...
import { Bell, Heart, Settings, LogOut } from 'lucide-react'
...
export function ProfileDropdown({ user }: ProfileDropdownProps) {
  ...
  return (
    <div className="relative" ref={wrapRef}>
      ...
      >
        <span className="flex h-7 w-7 items-center justify-center overflow-hidden rounded-full border-[1.5px] border-[var(--gold)] bg-[rgba(232,184,75,0.2)] text-[var(--gold)]">
          {user.image ? (
      ...
      {open && (
        ...
        >
          <div className="flex flex-col items-center bg-white/[0.03] px-5 pb-5 pt-6 text-center">
            <div className="mb-2.5 flex h-[60px] w-[60px] items-center justify-center overflow-hidden rounded-full border-2 border-[var(--gold)] bg-[rgba(232,184,75,0.15)] text-[var(--gold)]">
              {user.image ? (
                <img src={user.image} alt="" className="h-full w-full object-cover" />
              ) : (
                <AvatarIcon size={28} />
          ...
          <div className="p-2">
            <div className="flex w-full items-center gap-2.5 rounded-[10px] px-3 py-2.5">
              <Bell size={16} className="w-5 flex-shrink-0 text-white/70" />
            ...
            >
              <Heart size={16} className="w-5 flex-shrink-0 text-white/70" />
            ...
            <Link
              ...
              className="flex w-full items-center gap-2.5 rounded-[10px] px-3 py-2.5 text-sm text-white/85 no-underline transition-colors hover:bg-white/[0.06]"
            >
              <Settings size={16} className="w-5 flex-shrink-0 text-white/70" />
          ...
          >
            <LogOut size={16} /> <span>{t('profileMenu.logout')}</span>
          </button>
        </div>
      )}
    </div>
