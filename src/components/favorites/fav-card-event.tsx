      ...
      <div className="flex flex-1 items-start gap-4 p-5">
        ...
        <div className="flex-1">
          ...
          <p className="mb-1 flex items-center gap-1 text-sm text-[#6b7280]">
            <Calendar size={14} className="flex-shrink-0" /> {new Date(event.date).toLocaleDateString(lang === 'nl' ? 'nl-NL' : 'en-US')}
          </p>
          <p className="flex items-center gap-1 text-sm text-[#6b7280]">
            <MapPin size={14} className="flex-shrink-0" /> {event.isOnline ? t('openHouses.online') : event.location || t('favorites.noLocation')}
          </p>
        </div>
      </div>
    </div>
